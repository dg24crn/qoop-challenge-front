...
const Payment: React.FC = () => {
  ...
  const handleCardNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length > 16) value = value.slice(0, 16); // Limitar a 16 caracteres
    setCardNumber(value.replace(/(\d{4})(?=\d)/g, "$1 ")); // Agregar espacios cada 4 dígitos
  };

  const validateCard = () => {
    const isCardNumberValid = cardNumber.replace(/\s/g, "").length === 16; // Validar longitud sin espacios
    const isExpiryMonthValid = expiryMonth.length === 2 && Number(expiryMonth) >= 1 && Number(expiryMonth) <= 12;
  ...
  const handlePayNow = async () => {
    ...
    try {
      // Llamar al endpoint para activar la suscripción Premium
      await axios.post(
        `http://127.0.0.1:8000/users/${user.id}/subscribe`,
        {},
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4">
      <h1 className="text-4xl font-bold text-gray-800 mb-8 text-center">Premium Plan</h1>
          ...
          <form className="flex flex-col gap-4">
            <div>
              ...
              <input
                ...
                maxLength={19} // Incluye los espacios
              />
            </div>
